...
export default function PopularPackageCarousel({
	packages,
}: { packages: PopularPackageInfo[] }) {
	if (!packages || packages.length === 0) {
		return (
			<div className="w-full p-4 text-center text-surface-medium">
				패키지 없음
			</div>
		);
	}
								...
								{(pkg.publisher?.email || pkg.author?.email) && (
									<>
										...
										<p className="text-base mt-2 text-surface-medium">
											{pkg.publisher?.username !== "Unknown"
												? pkg.publisher?.username
												: ""}
											{pkg.publisher?.username !== "Unknown" &&
