		...
		const loadSearchResults = async () => {
			try {
				...
				if (err instanceof Error && "status" in err) {
					searchError.status = (err as { status: number }).status;
				}
	...
	return (
		<div className="flex flex-col items-center">
			<div className="bg-secondary-90 w-80 max-h-[460px] flex flex-col rounded-[20px]">
				<div>
					<p className="ml-8 mt-6 text-xl font-semibold text-[#F5F6F8]">
						Related search
					</p>
						...
						>
							...
							>
								<div className="flex flex-col py-[5px] px-[6px]">
									...
									<p className="text-sm text-secondary-40 line-clamp-2">
										{result.snippet}
									</p>
									<div className="flex mt-1 ">
										...
										<p className="text-sm text-secondary-40 ml-1 ">
											{result.displayLink}
										</p>
										<ExternalLink
