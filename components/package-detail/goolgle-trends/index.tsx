...
import type { GoogleTrendsProps, TrendsData } from "@/types/google-trends";
...
export function GoogleTrends({ packageName }: GoogleTrendsProps) {
			...
			try {
				...
				const trendsError: TrendsError =
					err instanceof Error
						? err
						: new Error("Failed to fetch google trends data");
				if (err instanceof Error && "status" in err) {
					trendsError.status = (err as { status: number }).status;
				}
	...
	const calculateAverage = useMemo(() => {
		...
		const sum = trendsData.interest.reduce(
	...
	const calculateMax = useMemo(() => {
		if (!trendsData?.interest) return 0;
		const max = trendsData.interest.reduce(
		...
		return (
			<div className="w-[785px] bg-secondary-90 rounded-[20px] p-6 mb-6">
				<p className="text-xl font-semibold mb-2 text-[#ff000083]">
					We couldn’t find any google-trends information on this package. It may
	...
	return (
		<>
			{trendsData?.interest?.length ? (
				<div className="w-[785px] h-full">
					<div className="w-full bg-secondary-90 rounded-[20px] p-6 mb-6">
						...
						<p className="text-[#F6F6F6] mb-4">
							Numbers represent relative search interest where 100 is the peak
							popularity for the term. A value of 50 means it is half as
							popular, and 0 means insufficient data.
						</p>
					</div>
						...
						<div className="h-[352px] w-[661px] ml-14 mt-3 rounded-[20px] bg-white">
										...
										dot={(props) => {
											...
											return isMax ? (
												<svg
													...
													y={props.cy - 7}
												>
													<title>Maximum Interest Point</title>
													<circle
														cx="7"
														cy="7"
														r="6.5"
							...
							<div className="text-sm pb-4 text-gray-500 flex justify-between items-center">
								...
								<span>{`${trendsData.interest[0].formattedAxisTime} - ${
									trendsData.interest[trendsData.interest.length - 1]
										.formattedAxisTime
								}`}</span>
				...
				<div className="w-[785px] bg-secondary-90 rounded-[20px] p-6 mb-6">
					<p className="text-xl font-semibold mb-2 text-[#ff000083]">
						We couldn’t find any google-trends information on this package. It
